...
- **s2 has only 12 items for Hindi** (Urdu has 5) — thin for the stage the ladder calls "the
  core abugida skill".
...
### Phase A — Tidy the seams *(small, do first)*

Retire the `IND_HI_*` special-casing so all nine packs reach sentences and dialogues through
`IND_BHASHA_SENTENCES[packId]` / `IND_BHASHA_DIALOGUES[packId]`. Record a `reviewer` field in
every pack file per [09 §9]. Thicken Hindi s2.
...
*Done when:* `sentenceMap()` has no pack-id branch, and `tools/test-bhasha.js` asserts it.
